	...
	if (event == CV_EVENT_FLAG_LBUTTON)
	{
		...
		if (x != -1 && y != -1)
		{
			...
			if (depth != 0)
			{
				...
				p3D[*id].X = -1;
				circle(depthImages[*id], Point(x,y), 2, Scalar(255,0,0));
				imshow(nWindows[*id], depthImages[*id]);
				waitKey(0);
			}
...
*/
	...
	/*				if (frames == debugFrame)
		...
		//for (int i = 0; i < 480; i++)
		//{
		//	int depth = depthMaps[0][i*XN_VGA_X_RES+615];
		//	if (depth != 0)
		//	{
		//		XnPoint3D edge2D[1];
		//		XnPoint3D edge3D[1];
		//		edge2D[0].X = 615; edge2D[0].Y = i; edge2D[0].Z = depth;
		//		kinects[0].arrayBackProject(edge2D, edge3D, 1);
		//		kinects[0].transformArrayNoTilt(edge3D, 1);
		//		XnPoint3D* edge2Dp = kinects[1].arrayProject(edge3D, 1);
		//		Point p = Point(edge2Dp->X, edge2Dp->Y);
		//		circle(depthImages[1], p, 2, Scalar(255,0,0));
		//		//XnPoint3D edge3D[1];
		//		//edge3D[0].X = depth*tanf(28.5*CV_PI/180);
		//		//float range = sqrtf( powf(depth,2) + powf(edge3D[0].X,2) );
		//		////edge3D[0].Y = range * tanf(19.09*CV_PI/180);
		//		//edge3D[0].Y = depth * tanf(21.5*CV_PI/180);
		//		//edge3D[0].Z = depth;
	
		//		//kinects[0].transformArrayNoTilt(edge3D, 1);
		//		//XnPoint3D* edge2D = kinects[1].arrayProject(edge3D, 1);
		//		//Point p = Point(edge2D->X, edge2D->Y);
		//		//circle(depthImages[1], p, 2, Scalar(255,0,0));

		//	}
		//}

		//line(depthImages[0], Point(615, 0), Point(615, 480), Scalar(0,0,255));
	
	//	line(depthImages[1], p1,p2, Scalar(0,0,255));
	//	line(depthImages[1], p3,p4, Scalar(0,0,255));

		line(depthImages[1], Point(121, 0), Point(241,480), Scalar::all(0));
		line(depthImages[1], Point(599, 0), Point(499,480), Scalar::all(0));
